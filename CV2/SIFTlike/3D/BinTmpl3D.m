function [ MFS ] = BinTmpl3D( sequence )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% compute the gradient of the image by 3D orientation
x_mask=zeros(2,2,2);
y_mask=zeros(2,2,2);
z_mask=zeros(2,2,2);
...
%% compute 2 orientations of the gradient by counting
...
for n = 1:nLevel
    for f=1:nframe
        ...
        for i = 1:orient_bin_num
            ...
            bin_up{f,i} = conv2(single(map),mask,'same')/ ((2*n+1)^2);  %normalize
            map = G_phi(:,:,f) >= -pi+(i-1)*orient_bin_step & G_phi(:,:,f) < -pi+i*orient_bin_step;
            bin_down{f,i} = conv2(single(map),mask,'same')/ ((2*n+1)^2);  %normalize
        end
    end
   
    for f=1:nframe
        %%% Bin to MFS
        temp_bin_up=zeros(row,col,orient_bin_num);
        temp_bin_down=zeros(row,col,orient_bin_num);
        for m = 1:orient_bin_num
            temp_bin_up(:,:,m) = bin_up{f,m};
            temp_bin_down(:,:,m) = bin_down{f,m};
        end
        [tmp_row,tmp_col]=size(bin_up{1,1});
        descriptor_up = reshape(temp_bin_up,[tmp_row*tmp_col,orient_bin_num]);
        descriptor_down = reshape(temp_bin_down,[tmp_row*tmp_col,orient_bin_num]);
        bitMap_up = descriptor_up >= 0.1;
        bitMap_down = descriptor_down >= 0.1;
    ...
    for p=1:tempLen
        ...
        if sum(sum(temp_seq_bw_up ~= 0)) < nZero
            binMFS_up((n-1)*tempLen + p) = 0;
        else
            binMFS_up((n-1)*tempLen + p) = bw2MFS3D(temp_seq_bw_up,nEst);
        end
        if sum(sum(temp_seq_bw_down ~= 0)) < nZero
            binMFS_down((n-1)*tempLen + p) = 0;
        else
            binMFS_down((n-1)*tempLen + p) = bw2MFS3D(temp_seq_bw_down,nEst);
        end
    end
    clear temp_seq_bw_up temp_seq_bw_down
end
MFS=[binMFS_up,binMFS_down];