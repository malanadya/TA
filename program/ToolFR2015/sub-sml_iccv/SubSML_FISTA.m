function  result = SubSML_FISTA(Xtr, SS, DS, ggamma, beta, di, options)
...
% ggamma, beta: regularization parameter
...
[~, nf] = size(Xtr);
% project to the intra-personal subspace
ut = ones(nos_sim,1); 
...
%   compute f(xp)'(=gradfxp)  
        ...
        SStemp_M = SODW(Xtr', SS(:,1), SS(:,2), xp(1:nos_sim));
        DStemp_M = SODW(Xtr', DS(:,1), DS(:,2), xp(1+nos_sim:end)); 
        gradfyp_M = SStemp_M- DStemp_M;
        SStemp_G = SODW_SIM(Xtr', SS(:,1), SS(:,2), xp(1:nos_sim));
        DStemp_G = SODW_SIM(Xtr', DS(:,1), DS(:,2), xp(1+nos_sim:end)); 
        gradfyp_G = SStemp_G- DStemp_G;
        
        M_xp = Id-(1/ggamma).*gradfyp_M;
        M_xp = (M_xp+M_xp')./2;
        G_xp = Id+(1/beta).*gradfyp_G;
        G_xp = (G_xp+G_xp')./2;
%         
        H1_POS = -Score_SML(Xtr, SS(:,1), SS(:,2), M_xp, G_xp); % minus of similarity scores for the positive pairs
        H1_NEG = Score_SML(Xtr, DS(:,1), DS(:,2), M_xp, G_xp); % similarity scores for the negative pairs
        a1 = 1+ [H1_POS; H1_NEG];     
        gradfxp = -a1;
        alphap = alpha;
        
        f_xp = sum(xp.*a1)+ (1/(2*ggamma))*trace((gradfyp_M)^2)+ (1/(2*beta))*trace((gradfyp_G)^2); 
        % f_xp = sum(a_ij(1+ y_ij((x_i-x_j)'*M*(x_i-x_j)-x_i'*G*x_j)))+
        % (ggamma/2)|M- I|^2,
        % (ggamma/2)|M- I|^2 = (1/(2*ggamma))|gradfyp_M|^2
        f_xp = -f_xp;
    ...
    while (1)
     % compute fnew(f(alpha)), f(xp) and ftaylor  
        ...
        SStemp_M = SODW(Xtr', SS(:,1), SS(:,2), alpha(1:nos_sim));
        DStemp_M = SODW(Xtr', DS(:,1), DS(:,2), alpha(1+nos_sim:end)); 
        gradfap_M = SStemp_M- DStemp_M;
        ...
        M_alpha = Id-(1/ggamma).*gradfap_M;
        M_alpha = (M_alpha+M_alpha')./2;
        ...
        Fnew = sum(alpha.*a2)+ (1/(2*ggamma))*trace((gradfap_M)^2)+ (1/(2*beta))*trace((gradfap_G)^2);
        Fnew =-Fnew;
