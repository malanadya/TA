function  result = SubML_FISTA(Xtr, SS, DS, ggamma, di, options)
...
% ggamma: regularization parameter
...
%%initializing
f = size(Xtr,2);
Id=eye(f);ut1=ones((nos_sim+ nos_dsim), 1);%eye(nf)
...
%   compute f(xp)'(=gradfxp)  
        ...
        G_xp = zeros(f);       
        H1_POS = -Score_SML(Xtr, SS(:,1), SS(:,2), M_xp, G_xp); % minus of similarity scores(-dist) for the negative pairs
        ...
        f_xp= sum(xp.*a1)+ (1/(2*ggamma))*trace((gradfyp_M)^2);
        f_xp=-f_xp;
...
%         M_alpha=V*D*V';
        ...
        Fnew= sum(alpha.*a2)+ (1/(2*ggamma))*trace((gradfap_M)^2);
        Fnew=-Fnew;
